...
#Select the fourth Th atom
    ...
    while not (system.get_type(fourth_id)==2 and get_distance(system.get_position(fourth_id),system.get_position(third_id))<=4.0 and get_distance(system.get_position(fourth_id),system.get_position(second_id))<=4.0 and fourth_id not in selected_list):
        if (system.get_type(fourth_id)==2 and get_distance(system.get_position(fourth_id),system.get_position(third_id))<=4.0 and get_distance(system.get_position(fourth_id),system.get_position(atom_id))<=4.0 and fourth_id not in selected_list):
            break
        elif (system.get_type(fourth_id)==2 and get_distance(system.get_position(fourth_id),system.get_position(second_id))<=4.0 and get_distance(system.get_position(fourth_id),system.get_position(atom_id))<=4.0 and fourth_id not in selected_list):
            break
        else:
            fourth_id = random.randint(1,tot_num)
...
# Select the fifth Th atom
    ...
    for id in range(1,tot_num+1):
        if system.get_type(id) == 1:
            ...
            pass
    five_elem_tuple = (atom_id, second_id, third_id, fourth_id, fifth_id)
    Th_elem_id.append(five_elem_tuple)
key_list = []
...
for i in Th_elem_id:
    ...
    while True:
        ...
        if index not in index_ls:
            index_ls.append(index)
        if len(index_ls) == 2:
            break
    
    Xe_id.append(i[index_ls[0]])
    Xe_id.append(i[index_ls[1]]) 
    
print(Th_id_array)
print(Oxygen_id_array)
print('tuple with five elements: ', len(Th_elem_id), np.array(Th_elem_id))
print('Xe list: ', len(Xe_id), np.array(Xe_id))
